...
export default function InstitutionAdminLogin({ onLogin }: { onLogin: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      const { data } = await axios.post('/api/institution-admin/login', { email, password });
      localStorage.setItem('institutionAdminToken', data.token);
      localStorage.setItem('institutionAdminInfo', JSON.stringify(data.admin));
      onLogin();
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
      <div className="bg-white rounded-2xl shadow-lg p-8 max-w-sm w-full">
        ...
        <h1 className="text-xl font-bold text-slate-800 text-center mb-6">Kurum Yönetici Girişi</h1>
